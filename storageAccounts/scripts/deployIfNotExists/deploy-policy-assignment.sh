...
# Variables
policyAssignmentName="storage-account-pe-should-use-pdzg-policy-assignment"
policyDefinitionName="storage-account-pe-should-use-pdzg-policy-definition"
location="WestEurope"

# ARM template and parameters files
template="../../templates/deployIfNotExists/azuredeploy.policy.assignment.json"
parameters="../../templates/deployIfNotExists/azuredeploy.policy.assignment.parameters.json"
...
# Sleep 10 seconds waiting for the change to propagate
seconds=30
...
for roleDefinitionId in ${roleDefinitionIds[@]}; do
    ...
    if [[ $roleDefinitionId != /subscriptions* ]]; then
